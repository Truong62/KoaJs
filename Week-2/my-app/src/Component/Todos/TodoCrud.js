...
export const useTodoCrud = () => {
    const {datas} = useFetchApi("http://localhost:5000/api/todolist/");
    const addTodo = async text => {
        let id = Math.floor(Math.random() * 1000);
        while (datas.some(todo => todo.id === id)) {
            id = Math.floor(Math.random() * 1000);
        }
        await fetchApi("http://localhost:5000/api/todolist", "POST", {id, name: text, "isCompleted": false})
    };
    const updateTodo = async id => {
        ...
        await fetchApi("http://localhost:5000/api/todolist/" + id, "PUT", {
            id: todoById.id,
            name: todoById.name,
            "isCompleted": true
        })
    ...
    const deleteTodo = async id => {
        ...
        if (newData.length > 0) {
            await fetchApi("http://localhost:5000/api/todolist/" + id, "DELETE")
        }
    };
